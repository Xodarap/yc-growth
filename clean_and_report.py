...
FABRICATED = {
    ...
    ("Rimward", 2025), ("Rimward", 2026),
    ("AtlasGrid", 2025), ("AtlasGrid", 2026),
    ("Piggy Robotics", 2025), ("Piggy Robotics", 2026),
...
ESTIMATE_PAT = re.compile(
    ...
    r"|\(estimated\)|estimated range", re.IGNORECASE)
...
def load(db, cutoff, to_date, clean=True):
    ...
    df = pd.read_sql_query(f"""
        ...
        WHERE c.status = 'completed' AND c.yc_year < {cutoff}""", conn)
    conn.close()
    dropped = {}
    if clean:
        ...
        dropped = {"synthetic estimates": int(est.sum()), "verified fabrications": int(fab.sum())}
        df = df[~est & ~fab].copy()
    df["valuation_numeric"] = df.valuation.apply(parse_valuation)
    return df[df.valuation_numeric.notna()].copy(), dropped


def marks(numeric, to_date, dedup=True, strict_one_year=False):
    inflators = {y: cpi.inflate(1, date(int(y), 6, 1), to=to_date) for y in numeric.year.unique()}
    out = {}
    for lag, strict in ((2, True), (1, True if strict_one_year else False)):
        rows = [g for company in numeric.company.unique()
                if (g := lagged_valuation(numeric[numeric.company == company], lag, inflators, strict))]
        df = pd.DataFrame(rows)
        if dedup and len(df):
            df["key"] = df.company.map(norm)
            df = (df.sort_values("valuation_real", ascending=False)
...
def compare(label_a, frames_a, label_b, frames_b, era=2023):
    ...
    for lag in (2, 1):
        ...
        for label, frames in ((label_a, frames_a), (label_b, frames_b)):
            ...
            _, p = stats.mannwhitneyu(recent.valuation_real, older.valuation_real,
                                      alternative="two-sided")
            _, pl = stats.ttest_ind(np.log10(recent.valuation_real[recent.valuation_real > 0]),
            ...
            rowsets.append({
                ...
                f"median pre-{era}": format_valuation(older.valuation_real.median()),
        ...
        for label, frames in ((label_a, frames_a), (label_b, frames_b)):
            ...
            for i, (_, r) in enumerate(top.iterrows(), 1):
                ...
                lines.append(f"  {i:>2}{star} {r.company:<26} {r.batch:<13} "
...
if __name__ == "__main__":
    ...
    ap.add_argument("--raw", action="store_true", help="skip cleaning (reproduce the naive pipeline)")
    a = ap.parse_args()
    clean = not a.raw
    old_num, _ = load("yc_valuations.db", 2025, date(2025, 6, 1), clean=clean)
    new_num, dropped = load("yc_valuations_2026.db", 2026, date(2026, 6, 1), clean=clean)
    ...
    for strict in (False, True):
        print(f"\n\n{'#'*96}\n# 1-YEAR WINDOW: "
              ...
              f"\n{'#'*96}")
        print(compare("baseline Aug-2025", marks(old_num, date(2025, 6, 1), clean, strict),
                      "re-run Sep-2026", marks(new_num, date(2026, 6, 1), clean, strict)))
